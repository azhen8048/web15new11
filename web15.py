...
# ==============================================================================
st.set_page_config(
    page_title="基于机器学习的早发心梗后心衰中西医结合预测模型",
    page_icon="❤️", 
    layout="wide"
)
...
def main():
    ...
    ## 关于本系统
    这是一个基于机器学习的**早发心肌梗死后心力衰竭**中西医结合预测系统，用于评估患者发生心衰的风险。
    ...
    # 主页面标题
    st.title("基于机器学习的早发心肌梗死后心力衰竭中西医结合预测模型")
    ...
    if predict_button:
        ...
        # 确保 input_df 中的数据类型为数字，防止模型或SHAP报错
        ...
        try:
            ...
            try:
                ...
                # 创建一个用于SHAP显示的中文标签数据框（只包含字符串）
                display_data_labeled = input_df.iloc[0].copy()
                to_cn_map = {0: '否', 1: '是'}
                try:
                    # 仅对类别/二值变量进行中文映射
                    binary_features = ['Outcome_CHD_DM', 'Outcome_feiyan', 'Tachyarrhythmia', 'TCM', 'qizhixueyu', 'yangxu', 'xueyushuiting']
                    for feature in binary_features:
                        if feature in display_data_labeled.index:
                            display_data_labeled.loc[feature] = to_cn_map.get(int(display_data_labeled.loc[feature]), str(display_data_labeled.loc[feature]))
                except Exception:
                    pass
                ...
                try:
                    ...
                    # 核心: data 参数传递原始的 NUMERIC 数据 (input_df.iloc[0].values)，SHAP才能正常工作
                    ...
                    try:
                        shap.waterfall_plot(
                            shap.Explanation(
                                ...
                                data=input_df.iloc[0].values, # 传递原始数值
                ...
                try:
                    ...
                    # 力图使用包含中文标签的 display_data_labeled
                    force_plot = shap.force_plot(
                        expected_value,
                        shap_value,
                        display_data_labeled, # 使用包含中文标签的 Series
                        feature_names=[feature_dict.get(f, f) for f in current_features]
                    )
